...
namespace JuliEngine
{
	...
	void Entity2::SetRot(glm::vec3 rotation)
	{
		getTransform()->setRot(rotation);
	}

	void Entity2::SetRotRadians(glm::vec3 rot)
	{
		getTransform()->setRotRadians(rot);
	}

	void Entity2::SetRotX(float x)
	{
		getTransform()->setRotX(x);
		updateModelMatrix();
	}

	void Entity2::SetRotY(float y)
	{
		getTransform()->setRotY(y);
		updateModelMatrix();
	}

	void Entity2::SetRotZ(float z)
	{
		getTransform()->setRotZ(z);
		updateModelMatrix();
	}
...
>>>>>>> Stashed changes
	void Entity2::DebugInfo()
	{
		...
		cout << "sca: " + VecToString::vec3toString(transform->getScale())		<< std::endl;
	...
	void Entity2::UseLocalMatrix()
	{
		getTransform()->UseLocalMatrix();
	}
	...
	void Entity2::setDraw()
	{
		...
		for (int i = 0; i < getChildren().size(); i++)
		{
			...
			getChildren()[i]->setWorldModelWithParentModel(getTransform()->getModel());
	...
	void Entity2::setWorldModelWithParentModel(glm::mat4 parentModel)
	{
		...
		getTransform()->setWorldModelWithParentModel(parentModel);
